...
var _ = Describe("Hotfix", func() {
	...
	It("closes on release branches & dev branch", func() {
		ForEachTestSet(branches, func(branch interface{}) {
			...
			Expect(closeBanches[1].ShortBranchName()).To(Equal(DEV_BRANCH))
		})
	})

	It("is only allowed to create from main branch", func() {
		ForEachTestSet(branches, func(branch interface{}) {
			f := branch.(Branch)
			for _, testBranch := range MockBranchCollection() {
				testBranchName := testBranch.ShortBranchName()
				if testBranchName == MAIN_BRANCH {
	...
	It("can be published on main branch", func() {
		ForEachTestSet(branches, func(branch interface{}) {
			publishBranch := branch.(Branch).PublishBranch()
			Expect(publishBranch.ShortBranchName()).To(Equal(MAIN_BRANCH))
		})
	})
